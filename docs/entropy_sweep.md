...
`scripts/runs/run_entropy_sweep.sh` scans `actor_rollout_ref.actor.entropy_coeff` on Sokoban.
...
- Project name: `ragen_release_entropy_sweep`
...
- Per-run logs: `logs/entropy_sweep_Qwen2.5-3B/<filter_tag>/<value-label>/`
- Checkpoints: `model_saving/entropy_sweep_Qwen2.5-3B/<value-label>/`
